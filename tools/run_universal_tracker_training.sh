...
if [[ ! -f "$RUN_ROOT/dagger/collect_round0.json" ]]; then
  ...
  echo "Skipping completed DAgger collection round 0"
fi
...
if [[ ! -f "$RUN_ROOT/dagger/model_round0.pt" ]]; then
  ...
  echo "Skipping completed DAgger training round 0"
fi
...
if [[ ! -f "$RUN_ROOT/dagger/collect_round1.json" ]]; then
  "$PYTHON" -u "$ROOT/tools/collect_dynamics_dagger.py" \
    --task g1_dynamics_tracker_universal \
    --motion_file "$MOTIONS" \
    --dataset "$RUN_ROOT/dagger/replay.pt" \
  ...
  echo "Skipping completed DAgger collection round 1"
fi
...
if [[ ! -f "$RUN_ROOT/dagger/model_round1.pt" ]]; then
  "$PYTHON" -u "$ROOT/tools/train_dynamics_dagger.py" \
    --task g1_dynamics_tracker_universal \
    --dataset "$RUN_ROOT/dagger/replay.pt" \
    --template-checkpoint "$TEMPLATE" \
  ...
  echo "Skipping completed DAgger training round 1"
fi

if [[ ! -d "$RUN_ROOT/ppo" ]]; then
  ...
  echo "PPO directory already exists; use an explicit --resume-checkpoint command"
fi
